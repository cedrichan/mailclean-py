...
def main():
    parser = argparse.ArgumentParser(description="Remove original emails marked with PRE_CLEANUP.")
    parser.add_argument("--dry-run", action="store_true", help="List messages without deleting them.")
    ...
    for msg_meta in messages_to_process:
        ...
        msg_data = service.users().messages().get(userId='me', id=msg_id, fields='labelIds,snippet').execute()
        label_ids = msg_data.get('labelIds', [])
        snippet = msg_data.get('snippet', '')

        if post_label_id and post_label_id in label_ids:
            print(f"Skipping {msg_id} - safety check failed: message also has POST_CLEANUP label.")
            continue
